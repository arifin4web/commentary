...
from feeds.consumers import connect_event, disconnect_event
...
channel_routing = [
    ...
    route("websocket.connect", connect_event, path=r'^/event/(?P<slug>[^/]+)/stream/$'),

    # Called when the client closes the socket
    route("websocket.disconnect", disconnect_event, path=r'^/event/(?P<slug>[^/]+)/stream/$'),
